...
def test_requests():
    ...
    for request, body in request_bodies.items():
        ...
        assert response_time < 25, f"TEST Failed timeout: The {request} API has failed. Please check with the Big Boss."

        # check status code
        assert response.status_code == 200, f"TEST Failed not 200: The {request} API has failed. Please check with the Big Boss."
        # Test the status
        json_response = response.json()
        assert 'errors' in json_response, f"TEST Failed error in response: The {request} API has failed. Please check with the Big Boss."
